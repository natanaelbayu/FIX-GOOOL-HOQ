...
# TAB 3 (Correlation - The Roof Top & Side)
with t3:
    col_side, col_top = st.columns(2)
...
# --- 🏛️ TAB 6: FULL HOQ ARCHITECTURE (DENGAN ROOF ATAS + ROOF SAMPING KIRI) ---
with t6:
    try:
        st.write("### 🏛️ Arsitektur Matriks House of Quality (HoQ) Komplit + Roof Kiri")
        st.caption("Visualisasi rumah kualitas mengintegrasikan Atap Atas (HOWs vs HOWs) dan Atap Samping Kiri (WHATs vs WHATs).")
        ...
        n_side_cols = n_whats - 1  # Jumlah grid kolom untuk atap samping kiri
        
        # Box Keterangan Simbol (Legend) - FIX CONCAT STRING UNTUK MENCEGAH SYNTAX ERROR TRIPLE QUOTES
        legend_html = (
        ...
        for i in range(n_hows - 1):
            html_hoq += '<tr>'
            # Membuat space kosong transparan di kiri (melompati area atap samping kiri + kolom WHATs + Importance)
        ...
        # Garis pembatas tipis antara atap segitiga atas dengan kepala tabel utama
        ...
        # -------------------------------------------------------------
        html_hoq += '<tr>'
        # Header untuk kolom Atap Samping Kiri
        ...
        # -------------------------------------------------------------
        # 3. GENERATE BADAN MATRIKS UTAMA + ATAP SAMPING KIRI (WHATs vs WHATs)
        # -------------------------------------------------------------
        for idx, row_name in enumerate(whats_list):
            imp_val = weights[idx]
            html_hoq += '<tr>'
            
            # GENERATE CELL ATAP SAMPING KIRI (Segitiga Samping)
            ...
            # Kolom Badan Hubungan Utama (Simbol ◎, ○, △)
            for col_name in hows_list:
                score_val = st.session_state.rel_matrix.at[row_name, col_name]
        ...
        # -------------------------------------------------------------
        # 4. GENERATE FONDASI RUMAH HOQ (ALIGN DENGAN STRUKTUR KOLOM)
        # -------------------------------------------------------------
