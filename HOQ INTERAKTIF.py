...
# --- BAGIAN PARSING LOGO BASE64 LOKAL ---
file_baca_logo = "ELITE UG (Original).png"
img_html = ""

# Memeriksa apakah file ada di direktori server, lalu mengonversinya ke Base64 HTML string
if os.path.exists(file_baca_logo):
    with open(file_baca_logo, "rb") as image_file:
        encoded_string = base64.b64encode(image_file.read()).decode()
    img_html = f'<img src="data:image/png;base64,{encoded_string}" width="90" style="display: block; border-radius: 8px;">'
else:
    # Backup emoji bangunan jika file secara tak terduga hilang agar layout tidak berantakan
    img_html = '<span style="font-size: 50px;">🏛️</span>'

# Render Header menggunakan HTML Flexbox murni agar posisi teks dan gambar selalu presisi sejajar
st.markdown(f"""
<div class="header-container">
    ...
    <div class="text-box">
        <p class="main-header">Digital House of Quality (HoQ)</p>
        <p class="sub-header">Optimalisasi Spesifikasi Teknis berdasarkan Voice of Customer</p>
    </div>
</div>
""", unsafe_allow_html=True)
...
# --- 🏛️ TAB 6: FULL HOQ ARCHITECTURE (DENGAN EXPORT HTML REPORT) ---
with t6:
    try:
        ...
        # GENERATE KEPALA TABEL BADAN UTAMA
        html_hoq += '<tr>'
        ...
        # Render tampilan utama di layar Streamlit
        st.markdown(html_hoq, unsafe_allow_html=True)
        
        # =========================================================================
        # BLOK IMPLEMENTASI OPSI 1: MEMBUNGKUS VARIABEL KE FORMAT DOKUMEN HTML UTUH
        # =========================================================================
        html_full_export = f"""
        <!DOCTYPE html>
        <html>
        <head>
            <meta charset="utf-8">
            <title>Laporan Digital House of Quality (HoQ)</title>
            <style>
                body {{ font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; padding: 30px; background-color: #ffffff; }}
                .hoq-table {{ border-collapse: collapse; margin: 20px auto; width: auto; box-shadow: 0 4px 6px rgba(0,0,0,0.05); }}
                .hoq-table th, .hoq-table td {{ border: 1px solid #e5e7eb; padding: 12px; text-align: center; font-size: 13px; color: #1f2937 !important; }}
                .hoq-th-corner {{ background-color: #ede9fe !important; font-weight: 700; color: #4c1d95 !important; }}
                .hoq-th-hows {{ background-color: #fce7f3 !important; color: #9d174d !important; font-weight: 700; }}
                .hoq-importance-header {{ background-color: #ccfbf1 !important; color: #115e59 !important; font-weight: 700; }}
                .hoq-td-whats {{ background-color: #e0f2fe !important; text-align: left !important; font-weight: 600; color: #075985 !important; }}
                .hoq-importance {{ background-color: #fef3c7 !important; font-weight: 700; color: #92400e !important; }}
                .hoq-score-row {{ background-color: #ffedd5 !important; font-weight: bold; color: #9a3412 !important; border-top: 2px solid #fdba74; }}
                .hoq-weight-row {{ background-color: #fee2e2 !important; font-weight: bold; color: #991b1b !important; }}
                .roof-blank {{ background-color: transparent !important; border: none !important; }}
                .roof-cell {{ background-color: #f3f4f6; border: 1px solid #d1d5db !important; font-weight: bold; }}
                .legend-box {{ border: 1px solid #e5e7eb; padding: 15px; border-radius: 8px; background-color: #f9fafb; max-width: 800px; margin: 0 auto 20px auto; font-size: 13px; }}
            </style>
        </head>
        <body>
            <div style="text-align: center; margin-bottom: 25px;">
                <h2 style="color: #4f46e5; margin-bottom: 5px; font-size: 26px;">Arsitektur Rumah Quality (House of Quality)</h2>
                <p style="color: #6b7280; margin-top: 0; font-size: 14px;">Praktikum Perancangan Teknik Industri (PTI) 1</p>
                <hr style="border: 0; border-top: 2px solid #f3f4f6; width: 100px; margin: 15px auto;">
            </div>
            
            <div class="legend-box">
                <strong>ℹ️ Simbol Hubungan (Badan Utama):</strong> &nbsp;
                <span style="color: #9f1239;"><strong>◎</strong> Kuat (9)</span> &nbsp;|&nbsp; 
                <span style="color: #9a3412;"><strong>○</strong> Sedang (3)</span> &nbsp;|&nbsp; 
                <span style="color: #854d0e;"><strong>△</strong> Lemah (1)</span>
            </div>
        ...
        st.write("")
        # Membuat tombol download interaktif
        st.download_button(
            label="📥 Unduh Arsitektur HoQ (Format Cetak HTML)",
            data=html_full_export,
            file_name="Digital_HoQ_Architecture.html",
            mime="text/html"
        )
